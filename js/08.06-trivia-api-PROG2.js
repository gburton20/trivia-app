...
radioButtons.forEach(radBtn => radBtn.addEventListener('click', () => {
    currentDifficulty = radBtn.value;
}));

// Stretch goals 
// 1) Display the average score as a percentage rather than a decimal number.
// 2) Handle true/false questions as well as multiple choice ones. You'll have to change the buttons around depending on how many answers the question has.
// 3) Turn off event listeners if the user has already given a correct answer.
// 4) Automatically go to the next question if the user has given a correct answer. Optionally, add a message that a new question is incoming and a delay using setTimeout.
// 5) Automatically highlight the correct answer if the user has already chosen every wrong answer. Optionally, don't change the score for them if they click that correct answer.
// 6) Do not let the user get a new question until they're finished with the current one.
