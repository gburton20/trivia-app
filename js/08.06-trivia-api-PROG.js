...
getTriviaButton.addEventListener('click', () => {
    fetch(`https://opentdb.com/api.php?amount=1&category=${categoryMenu.value}&difficulty=${selectedRadioBtn.value}&type=multiple`)
    .then((response) => response.json())
        .then((data) => {
            ...
            choices.forEach((choice, i) => { // 'i' is the index position of the choice. We need this because we're shuffling the array and need the index as something to shuffle relative to.
                let temp = choice;
...
function evaluateChoice (event) {
    ...
    // Print out tries, score, average
    scoreBox.innerHTML = `Tries: ${tries} &nbsp; &nbsp; &nbsp; Score: ${score} &nbsp &nbsp &nbsp; Avg: ${average}`;
}
...
radioButtons.forEach(radBtn => {
    if(radBtn.checked) selectedRadioBtn = radBtn;
});
